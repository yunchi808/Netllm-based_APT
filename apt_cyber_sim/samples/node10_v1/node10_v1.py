...
nodes = {
    "Website": m.NodeInfo(
        services=[
            ...
            m.ListeningService("MySQL", allowedCredentials=["ReusedMySqlCred-web"]),
        ],
    ...
    "Website[user=monitor]": m.NodeInfo(
        ...
        vulnerabilities={
            "ExploitationOfRemoteServices": m.VulnerabilityInfo(
                description="azurecredential.txt file in home directory",
                type=m.VulnerabilityType.REMOTE,
                outcome=m.LeakedCredentials(
                    credentials=[m.CachedCredential(node="AzureStorage", port="HTTPS", credential="SASTOKEN1")]
                ),
    ...
    "GitHubProject": m.NodeInfo(
        ...
        vulnerabilities=dict(
            ContainerAndResourceDiscovery=m.VulnerabilityInfo(
                description="Sharepoint info, including public IP address",
                type=m.VulnerabilityType.REMOTE,
                outcome=m.LeakedNodesId(["Sharepoint"]),
                reward_string="Obtained Sharepoint and public IP information",
                cost=1.0,
            )
        ),
    ),
    ...
    "Sharepoint": m.NodeInfo(
        ...
        vulnerabilities=dict(
            ScanSharepointParentDirectory=m.VulnerabilityInfo(
                ...
                type=m.VulnerabilityType.REMOTE,
                outcome=m.LeakedCredentials(
                    credentials=[m.CachedCredential(node="AzureResourceManager", port="HTTPS", credential="ADPrincipalCreds")]
                ),
                rates=m.Rates(successRate=1.0),
    ...
    "AzureResourceManager": m.NodeInfo(
        ...
        vulnerabilities=dict(
            ...
            CredScanHomeDirectory=m.VulnerabilityInfo(
                description="azurecredential.txt file in home directory",
                type=m.VulnerabilityType.LOCAL,
                outcome=m.LeakedCredentials(
                    credentials=[
                        m.CachedCredential(node="AzureResourceManager[user=monitor]", port="HTTPS", credential="azuread_user_credentials")
                    ]
                ),
                reward_string="SSH: cat ~/azurecreds.txt (running as monitor) revealed Azure user credential!",
                cost=1.0,
    ...
    "AzureResourceManager[user=monitor]": m.NodeInfo(
        ...
        properties=[],
        vulnerabilities=dict(
            ScanCredInRegistry=m.VulnerabilityInfo(
                description="azurecredential.txt file in home directory",
                type=m.VulnerabilityType.LOCAL,
                outcome=m.LeakedCredentials(credentials=[m.CachedCredential(node="Website", port="MySQL", credential="ReusedMySqlCred-web")]),
                reward_string="SSH: cat ~/Websitecreds.txt (running as monitor) revealed Website user credential!",
                cost=1.0,
            )
        ),
    ),
    "AzureVM": m.NodeInfo(
        ...
        firewall=m.FirewallConfiguration(
            incoming=[m.FirewallRule("SSH", m.RulePermission.BLOCK, reason="internet incoming traffic blocked on the VM by NSG firewall")],
            outgoing=[],
        ),
    ),
    "client": m.NodeInfo(
