...
nodes = {
    "Website": m.NodeInfo(
        ...
        firewall=m.FirewallConfiguration(
            ...
            + [m.FirewallRule("su", m.RulePermission.ALLOW), m.FirewallRule("sudo", m.RulePermission.ALLOW)],
        ),
        value=100,
        ...
        vulnerabilities=dict(
            ...
            CredScanBashHistory=m.VulnerabilityInfo(
                ...
                outcome=m.LeakedCredentials(
                    credentials=[
                        m.CachedCredential(node="Website[user=monitor]", port="su", credential="monitorBashCreds")
                    ]
                ),
    ...
    "Website.Directory": m.NodeInfo(
        ...
        vulnerabilities=dict(
            NavigateWebDirectoryFurther=m.VulnerabilityInfo(
                ...
                type=m.VulnerabilityType.REMOTE,
                outcome=m.LeakedCredentials(
                    credentials=[m.CachedCredential(node="Website", port="MySQL", credential="ReusedMySqlCred-web")]
                ),
...
ENV_IDENTIFIERS = m.infer_constants_from_nodes(
    cast(Iterator[Tuple[NodeID, NodeInfo]], list(nodes.items())), global_vulnerability_library
)
