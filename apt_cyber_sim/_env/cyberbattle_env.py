...
from typing import NamedTuple, Optional, Tuple, List, Dict, TypeVar, cast
...
from apt_cyber_sim._env.defender import DefenderAgent
from apt_cyber_sim.simulation.model import PortName, PrivilegeLevel
...
Observation = TypedDict(
    'Observation', {
        ...
        'all_nodes_properties': numpy.ndarray,

        'all_nodes_conquer_state': numpy.ndarray,
...
class CyberBattleEnv(gym.Env):
    ...
    def __reset_environment(self) -> None:
        ...
        self.__all_nodes_profile = [0 for _ in range(self.__bounds.property_count * self.__bounds.maximum_node_count)]
        self.__all_nodes_conquer_state = []

        self.__node_list = [t[0] for t in list(self.__environment.network.nodes().items())]

        for node_id, node_data in self.__environment.nodes():
            #self.__discovered_nodes.append(node_id)    #默认发现所有节点
            #一开始只能看到已经占领的节点
            if node_data.agent_installed:
                self.__discovered_nodes.append(node_id)
            if node_data.agent_installed:
                self.__all_nodes_conquer_state.append(1)
            else:
                self.__all_nodes_conquer_state.append(0)
    ...
    def __init__(self,
        ...
        self.__non_negative_reward = non_negative_reward
        self.__step_cost = step_cost
        ...
        self.__node_list = []
        for node_tuple in initial_environment.network.nodes.items():
            self.__node_list.append(node_tuple[0])
        ...
        self.observation_space = spaces.Dict({
            ...
            # ---------------------------------------------------------
            # Extra global features used by APT dataset generation
            # ---------------------------------------------------------

            # Bitmask indicating which nodes are owned (over the full node list)
            'all_nodes_conquer_state': spaces.MultiBinary(maximum_node_count),

            # Flattened node-property matrix for discovered nodes (0/1 encoding)
            'all_nodes_properties': spaces.MultiBinary(maximum_node_count * property_count),
            ...
            'all_nodes_list': spaces.MultiDiscrete([1] * maximum_node_count),
    ...
    def apply_mask(self, action: Action, mask: Optional[ActionMask] = None) -> bool:
        ...
        # print('field_mask'+str(field_mask.shape))
    ...
    def _get_all_profile(self):
        return self.__all_nodes_profile

    def _get_all_conquer_state(self):
        return self.__all_nodes_conquer_state
    ...
    def __get_blank_observation(self) -> Observation:
        observation = Observation(
            ...
            all_nodes_conquer_state=numpy.array(self.__all_nodes_conquer_state, dtype=numpy.int32),
            all_nodes_properties=numpy.array(self.__all_nodes_profile, dtype=numpy.int32),
            all_nodes_list=numpy.array(self.__node_list),
    ...
    def __get_all_nodes_profile(self):
        vector = numpy.zeros(self.__bounds.property_count * self.__bounds.maximum_node_count)
        for node_index, node in enumerate(self.__discovered_nodes):
            properties_indices = list(self._actuator.get_discovered_properties(node))
            is_owned = self._actuator.get_node_privilegelevel(node) >= PrivilegeLevel.LocalUser
            if is_owned:
                # if the node is owned then we know all its properties
                # => -1 should be the default value
                vector[node_index * self.__bounds.property_count:(node_index + 1) * self.__bounds.property_count] = 0

            vector[[i + node_index * self.__bounds.property_count for i in properties_indices]] = 1

        return vector

    def __get_all_nodes_conquer_state(self):
        vector = numpy.zeros(self.__node_count, dtype=numpy.int32)
        for node_id, node_info in self._actuator.discovered_nodes():
            is_owned = self._actuator.get_node_privilegelevel(node_id) >= PrivilegeLevel.LocalUser
            if is_owned:
                node_index = self.__node_list.index(node_id)
                vector[node_index] = 1
        return vector
    ...
    def __observation_reward_from_action_result(self, result: actions.ActionResult) -> Tuple[Observation, float]:
        ...
        obs['all_nodes_conquer_state'] = self.__get_all_nodes_conquer_state()
        obs['all_nodes_properties'] = self.__get_all_nodes_profile()
    ...
    def step(self, action: Action) -> Tuple[Observation, float, bool, StepInfo]:
        ...
        try:
            ...
            elif self.__non_negative_reward:
                reward = max(-self.__step_cost, reward)
